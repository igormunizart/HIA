...
def convert_linked_libraries_to_relative():
    """Converte todos os caminhos de bibliotecas linkadas para caminhos relativos"""
    blend_file = bpy.data.filepath
    if not blend_file:
        return False  # Se o arquivo .blend não foi salvo, não faz nada

    converted = False
    for lib in bpy.data.libraries:
        if lib.filepath and not lib.filepath.startswith('//'):
            # Se o caminho for absoluto (não começa com //)
            relative_path = get_relative_path(lib.filepath)
            if relative_path != lib.filepath:
...
class DOWNLOADRIG_OT_download_and_link(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            # Importa a collection usando o caminho absoluto para garantir que funcione primeiro
            ...
            # Após importação bem-sucedida, converta para caminhos relativos
            convert_linked_libraries_to_relative()
...
class DOWNLOADRIG_OT_update(Operator):
    ...
    def execute(self, context):
        try:
            ...
            # Primeiro atualiza usando o caminho absoluto
            for lib in bpy.data.libraries:
                if lib.filepath == self.filepath:
                    lib.filepath = new_filepath
                    lib.reload()

            # Depois converte para caminhos relativos
            convert_linked_libraries_to_relative()
...
class DOWNLOADRIG_OT_convert_to_relative(Operator):
    bl_idname = "downloadrig.convert_to_relative"
    bl_label = "Converter para Caminhos Relativos"
    bl_description = "Converte todos os arquivos linkados para usar caminhos relativos"
    ...
    def execute(self, context):
        ...
        converted = convert_linked_libraries_to_relative()

        if converted:
            self.report({'INFO'}, "Caminhos convertidos para relativos com sucesso!")
        else:
            self.report({'INFO'}, "Todos os caminhos já são relativos ou não há arquivos linkados.")
...
class DOWNLOADRIG_PT_update_panel(Panel):
    ...
    def draw(self, context):
        ...
        # Adiciona botão para converter todos os caminhos para relativos
        if bpy.data.libraries:
            layout.operator("downloadrig.convert_to_relative", icon='FILE_REFRESH')
            layout.separator()
...
classes = (
    ...
    DOWNLOADRIG_OT_convert_to_relative,
